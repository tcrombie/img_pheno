  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      plotOutput(outputId = "overlay", width = 1000, height = 1000,
                 click = "overlay_click",
                 brush = "overlay_brush"),
      verbatimTextOutput("overlay_brush_info")
    )
  )
)
...
server <- function(input, output) {
  ...
  output$overlay <- renderPlot({
    ...
    well <- ed[1,5]
      ...
      ggplot(worm_dat_proc %>% dplyr::filter(Metadata_Plate == input$plates, Metadata_Well == ed[1,5])) +
             ...
             labs(x = "", y = "", title = paste(ed[1,5]), fill = "Model Selection") +
  ...
  # Print selected rows
  output$overlay_brush_info <- renderPrint({
    ...
    # get rows
    brushedPoints(as.data.frame(worm_dat_proc %>%
                    dplyr::filter(Metadata_Plate == input$plates, Metadata_Well == ed[1,5]) %>%
                    dplyr::select(Metadata_Plate, Metadata_Well, strain, model_select, ObjectNumber, AreaShape_Center_X, AreaShape_Center_Y)), input$overlay_brush, allRows = TRUE)
  })
